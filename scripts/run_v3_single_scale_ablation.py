...
OUTPUT_PATH = V3_DIR / "single_scale_ablation.csv"
SMOKE_PATH = V3_DIR / "single_scale_ablation_smoke.csv"

VARIANTS: dict[str, dict[str, Any]] = {
    "multi_scale_full": {
        "encoder_mode": "multi",
        "single_kernel_size": 5,
        "note": "Full multi-scale encoder with kernels from the main MS-LIF-TCN design.",
    },
    "single_k3": {
        "encoder_mode": "single",
        "single_kernel_size": 3,
        "note": "Single-scale encoder with kernel size 3.",
    },
    "single_k5": {
        "encoder_mode": "single",
        "single_kernel_size": 5,
        "note": "Single-scale encoder with kernel size 5.",
    },
    "single_k9": {
        "encoder_mode": "single",
        "single_kernel_size": 9,
        "note": "Single-scale encoder with kernel size 9.",
    },
}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run v3 MS-LIF-TCN single-scale encoder ablation.")
        ...
        for variant in args.variants:
            for seed in args.seeds:
                ...
                if not args.force and row_exists(output_path, dataset_key, variant, int(seed), budget["training_budget"]):
                    print(f"Skipping existing single-scale row: {dataset_key} {variant} seed{seed}")
                    continue
                row = run_one(dataset_key, variant, int(seed), args)
                append_csv_row(output_path, row)
                print(f"Saved single-scale row: {dataset_key} {variant} seed{seed} -> {row['macro_f1']:.4f}")
...
def run_one(dataset_key: str, variant: str, seed: int, args: argparse.Namespace) -> dict[str, Any]:
    ...
    run_name = f"v3_single_scale_{dataset_key}_{variant}_k{meta.context_len}_{target_mode}_seed{seed}{suffix}"
...
def make_config(config_path: str, variant: str, seed: int, args: argparse.Namespace) -> dict[str, Any]:
    ...
    model_cfg["encoder_mode"] = str(variant_spec["encoder_mode"])
    model_cfg["single_kernel_size"] = int(variant_spec["single_kernel_size"])
